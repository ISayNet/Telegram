...
public class ContactsController {
    ...
    public void checkAppAccount() {
        ...
        Account[] accounts;
        try {
            accounts = am.getAccountsByType("ru.isaynet.account");
        ...
        accounts = am.getAccountsByType("ru.isaynet.messenger");
        ...
        if (recreateAccount) {
            ...
            if (UserConfig.isClientActivated()) {
                try {
                    currentAccount = new Account("" + UserConfig.getClientUserId(), "ru.isaynet.messenger");
    ...
    public void deleteAllAppAccounts() {
        try {
            AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
            Account[] accounts = am.getAccountsByType("ru.isaynet.messenger");
            for (int a = 0; a < accounts.length; a++) {
                am.removeAccount(accounts[a], null, null);
            }
        } catch (Exception e) {
        ...
        Utilities.globalQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                /*if (schedule) {
                    try {
                        AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                        Account[] accounts = am.getAccountsByType("ru.isaynet.account");
                        boolean recreateAccount = false;
                        if (UserConfig.isClientActivated()) {
                            if (accounts.length != 1) {
                                FileLog.e("detected account deletion!");
                                currentAccount = new Account(UserConfig.getCurrentUser().phone, "ru.isaynet.account");
                                am.addAccountExplicitly(currentAccount, "", null);
                                AndroidUtilities.runOnUIThread(new Runnable() {
                                    @Override
                                    public void run() {
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.ru.isaynet.messenger.android.profile");
